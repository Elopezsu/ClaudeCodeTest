# ValuSignal 2026 — Inbound Value & Conversion Nurture

**Audience:** Warm inbound appraiser leads entering Pipedrive (lead magnets, website forms, website lesson, LinkedIn).
**Out of scope:** Career Launchpad / students (owned by the team's student lead) and sponsors (own Sponsor Inquiry sequence).
**Platform:** Pipedrive automated campaign (one universal sequence).
**Cadence:** every 5 days (push pace). 10 emails currently; extend toward the event as needed.
**Sender:** The ValuSignal Team (Hansel credited once in E1).
**Threading:** E2 onward send as replies in the same thread.
**Merge fields:** `{{first_name}}`, `{{unsubscribe}}`.
**Goal:** Value first, drive registration. Registering unlocks the **library immediately**. Live sessions, workshops, and tech demos happen **at the event (Sept 18–19)** with on-demand replays for **three months after**, not on registration day.
**HTML:** branded Template-B files `nurture-E1` … `nurture-E10` (teal header band + CTA, Arial). Hand to Deanne for Pipedrive.

> ⚠️ **E2 (flash sale) is TIME-BOXED — ends July 4.** Keep it active in the sequence only through July 4, then pause/remove it (sequence reverts to 9 evergreen emails). A lead who joins after July 4 must not receive a "sale ends July 4" email. Drop the real promo code/price into E2 before sending.
> ⚠️ Run every email through the Mailmeteor spam checker before deploying.

---

## E1 · Welcome + what to expect + toolkit
**Subject:** welcome in, {{first_name}}

{{first_name}},

Thanks for connecting with ValuSignal through our available channels. Glad you're here.

Over the next few weeks we'll send the resources appraisers actually use: UAD 3.6 prep, AI workflows, speaker previews, and more. Starting today with the free tools below:
- **Appraisal Pulse Check:** where your business stands
- **AI-Ready Appraiser Scorecard:** your AI readiness
- **Appraisal Resilience Score:** your UAD 3.6 exposure
- **Workflow Capacity Calculator:** what your time is worth

Run whichever fits what you're facing.

Want the quick story on why we built this? Hansel Dobbs (Chief Appraiser and founder of ValuSignal 2026) and Gynell Vestal (founder of the American Valuation Society) break down what ValuSignal is here: youtube.com/watch?v=5NSrhZpEGjs

Hit reply anytime; we read every one.

The ValuSignal Team

---

## E2 · Flash sale (TIME-BOXED — ends July 4)
**Subject:** flash sale ends July 4, {{first_name}}

{{first_name}},

A quick, time-sensitive one: our flash sale ends July 4.

Through July 4 you can lock in ValuSignal 2026 at the lowest price we'll offer, with the full event, AI workshops, live tech demos, and three months of on-demand replays included. Use code EARLYBIRD at checkout.

Register before July 4: valusignal.com/register

After that it's back to full price. If you've been on the fence, this is your window.

The ValuSignal Team

---

## E3 · The library = instant reward for registering 🎯
**Subject:** what you unlock the day you register

{{first_name}},

Most people don't realize this: you don't have to wait until September to get value from ValuSignal 2026.

The moment you register, you get the ValuSignal Library, full of recorded sessions, lessons, and tools you can start using right away.

Then Sept 18–19 you get the live event, and everything stays on-demand for three months after. Register once, value starts today: valusignal.com/register

The ValuSignal Team

---

## E4 · Instant value (website lesson)
**Subject:** rebuild your site in 15 minutes (free)

{{first_name}},

Quick one that pays for itself: a free 15-minute lesson on how to rebuild your appraiser website with AI. No developer, no monthly tech fees.

Most appraisers pay hundreds a year for a site they could own outright. Watch it here: valusignal.com/website-lesson

It's a taste of the practical, do-it-today content ValuSignal is built around.

The ValuSignal Team

---

## E5 · Speakers + agenda
**Subject:** the Appraisal Foundation's president is speaking

{{first_name}},

This is the conversation at the top of the profession, no plane ticket required. A few of the names on the Sept 18–19 lineup:
- Kelly Davids, President, The Appraisal Foundation
- Malinda Griffin, President, National Association of Appraisers
- Gynell Vestal, Founder, American Valuation Society
- Lisa Desmarais, VP of Appraisal Issues
- Tony Pistilli, Restb.ai
- Jake Lew, first GSE-verified UAD 3.6 vendor
- Roy Meyer, Appraiser Mastermind (AI track)
- Daniel Lindeman, 10,000+ appraisals completed
- Michelle Bradley, appraiser and educator
- John Dingeman, certified appraiser and instructor

…plus 80+ speakers confirmed. See the full lineup: valusignal.com/speakers

Register and your library opens immediately: valusignal.com/register

The ValuSignal Team

---

## E6 · Tech demos
**Subject:** see every tool you use, demoed live

{{first_name}},

One of the most popular parts of ValuSignal 2026: live demos of the software and tools appraisers actually use, all in one place, so you can compare them side by side without sitting through a dozen sales calls.

See the full demo lineup: valusignal.com/tech-demos

Demos run live on Sept 18–19, with replays in your on-demand access for three months after. Your library access starts the moment you register: valusignal.com/register

The ValuSignal Team

---

## E7 · UAD 3.6 readiness (timely)
**Subject:** ready for November 2, {{first_name}}?

{{first_name}},

UAD 3.6 goes live November 2. Most appraisers are waiting to see how it plays out, which usually means scrambling when it does.

Jake Lew's firm was the first GSE-verified UAD 3.6 vendor; his session covers exactly what lenders and AMCs will expect on the first submissions. That's detail you won't find in a standard CE.

Register now and start prepping in the library today, then go deep live on Sept 18–19: valusignal.com/register

Where are you on 3.6, ahead of it or still waiting? Reply and let us know.

The ValuSignal Team

---

## E8 · AI workflows (practical)
**Subject:** the AI tools appraisers are actually using

{{first_name}},

The AI track covers hands-on sessions like Claude Code and AI Board of Advisors: Smarter Business Decisions, practical AI appraisers are already using to cut hours per report. Roy Meyer of Appraiser Mastermind leads it.

Built for every level, whether you're AI-curious or already experimenting.

You'll join these live on Sept 18–19, with on-demand replays for three months after. Your ValuSignal Library access starts the moment you register: valusignal.com/register

P.S. already using AI in your workflow, or still on the fence? Reply and tell us where you're at.

The ValuSignal Team

---

## E9 · Revenue diversification + networking
**Subject:** more than one source of work

{{first_name}},

One theme appraisers keep returning to: leaning on a single source of work, and AMC volume, is getting risky. A big part of ValuSignal 2026 is about adding work beyond it.

You'll also be in the room with appraisers and vendors working through the same shift. The structured networking, led by Cindi Harris and Josh Wallitt of Wallitt Solutions, is the kind you usually only get by traveling.

If you've been meaning to register, this is the week to do it. Library access starts the moment you do: valusignal.com/register

The ValuSignal Team

---

## E10 · Final close (recap the value)
**Subject:** here's everything registering gets you, {{first_name}}

{{first_name}},

Closing the loop. What one registration gets you, today and through the fall:
- **Immediate ValuSignal Library access:** recorded sessions, lessons, and tools
- **The live event Sept 18–19:** 70+ sessions, live tech demos, networking
- **Everything on-demand** for three months after

Value starts now: valusignal.com/register

Not ready yet? Follow us on LinkedIn (linkedin.com/company/valusignal) and stay in the loop.

The ValuSignal Team

---

## Open confirms before deploy
- E2 flash sale: real promo **code + price**, and confirm the **July 4** end date.
- Website lesson length: 15 or 10 minutes?
- Spelling: Cindi Harris / Josh Wallitt / Wallitt Solutions.
- Titles: Michelle Bradley and John Dingeman (verify against valusignal.com/speakers).
